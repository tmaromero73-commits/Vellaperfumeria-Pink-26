...
const WhatsAppFloat: React.FC<{ cartItems: CartItem[], currency: Currency }> = ({ cartItems, currency }) => {
    const generateWhatsAppLink = () => {
        const phone = "34661202616";
        if (cartItems.length === 0) {
            return `https://wa.me/${phone}?text=Hola!%20Vengo%20de%20la%20tienda%20online.%20Me%20gustaría%20saber%20más%20sobre%20vuestros%20productos%20de%20estilismo%20y%20color.`;
        }
        
        let message = "¡Hola!%20He%20preparado%20este%20pedido%20en%20Vellaperfumeria.com:%0A%0A";
        let subtotal = 0;
        cartItems.forEach(item => {
            const itemTotal = item.product.price * item.quantity;
            subtotal += itemTotal;
            message += `•%20*${item.product.name}*%20(x${item.quantity})%20-%20${formatCurrency(itemTotal, currency)}%0A`;
        });
        
        const discount = subtotal >= 35 ? subtotal * 0.15 : 0;
        const total = subtotal - discount;
        
        message += `%0A*Subtotal:*%20${formatCurrency(subtotal, currency)}`;
        if (discount > 0) message += `%0A*Descuento%20Loyalty%20(15%):*%20-${formatCurrency(discount, currency)}`;
        message += `%0A*TOTAL%20A%20PAGAR:*%20${formatCurrency(total, currency)}`;
        message += `%0A%0A*DIRECCIÓN%20DE%20ENVÍO:*%0A(Escribe%20aquí%20tu%20dirección)%0A`;
    ...
    return (
        <a
            ...
            className="fixed bottom-8 right-8 z-[100] bg-black text-white p-5 rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center border-2 border-[#f78df6]/40"
        ...
        >
            <div className="relative">
                ...
                {cartItems.length > 0 && (
                    <span className="absolute -top-3 -right-3 bg-[#f78df6] text-black text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center border-2 border-white animate-pulse">
...
const App: React.FC = () => {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-white font-sans selection:bg-[#f78df6] selection:text-black antialiased">
            ...
            <WhatsAppFloat cartItems={cartItems} currency={currency} />
